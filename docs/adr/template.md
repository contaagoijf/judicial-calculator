# ADR NNN: _titulo curto da decisao_

**Date:** DD/MM/AAAA

**Status:** Proposed | Accepted | Deprecated | Superseded by ADR-NNN

## Context

Qual é o problema ou a força em jogo que exige esta decisão? Descreva o suficiente para que alguém sem o contexto da conversa original entenda por que isso precisou ser decidido.

## Decision

O que foi decidido, de forma direta e verificável (não "vamos considerar", mas "vamos fazer X").

## Consequences

O que fica mais fácil, o que fica mais difícil, e qual trade-off foi aceito conscientemente.

---
> Copie este arquivo para `docs/adr/NNN-descricao-curta-em-kebab-case.md`, não use acentos no nome do arquivo, usando o próximo número sequencial de 3 dígitos disponível em `docs/adr/`.
