...
- [5. Referências e materiais relacionados](#5-referências-e-materiais-relacionados)
...
Consulte a seção [Acesso ao Sistema](#6-acesso-ao-sistema) para o passo a passo de login administrativo, primeiro acesso e convite de novos administradores.
...
- **SQL / PL/pgSQL**: funções, triggers e políticas de segurança do banco de dados (`supabase/schema.sql`).
- **Framework de UI**: React 18, com roteamento via `react-router-dom` (rotas em `src/App.tsx`, com *code splitting* por página via `React.lazy`).
- **Build / dev server**: Vite 5, com plugin `@vitejs/plugin-react-swc`. Configuração em `vite.config.ts`.
- **Estilização**: Tailwind CSS, com biblioteca de componentes shadcn/ui (baseada em Radix UI) em `src/components/ui/`.
- **Gerenciamento de estado assíncrono/cache**: TanStack Query (`@tanstack/react-query`), usado nos hooks de acesso a dados como `src/hooks/useIRData.ts`, `src/hooks/useRetificacaoContexto.ts` e `src/hooks/useSystemSettings.ts`.
- **Formulários e validação de esquema**: `react-hook-form` e `zod`.
- **Geração de PDF**: `jspdf` e `jspdf-autotable` (`src/services/pdfGenerator.ts`).
- **Gráficos** (quando aplicável na UI): `recharts`.
- **Testes**: Vitest para testes unitários (`src/test/`) e Playwright para testes end-to-end (`playwright.config.ts`, `playwright-fixture.ts`).
...
- **Hospedagem/deploy**: Vercel, como aplicação estática *single-page* (rewrite de todas as rotas para `index.html`, ver `vercel.json`).
...
- `src/integrations/supabase/client.ts` — cliente "gerado" a partir de variáveis de ambiente (`VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`), usado como referência/base para outros ambientes Supabase.
- `src/integrations/supabase/externalClient.ts` — aponta diretamente para o projeto Supabase de produção do CALCJUD e é o cliente efetivamente utilizado pela aplicação (autenticação, tabelas auxiliares, cálculos, configurações do sistema).

A **lógica de negócio do cálculo do IRPF é 100% front-end**, implementada de forma determinística em `src/services/calculoIRPF.ts`:
...
- **Unitários** (Vitest): validam principalmente as regras de cálculo determinísticas de `src/services/calculoIRPF.ts`. Rodar com `bun run test` (ou `npm run test`); `test:watch` para modo interativo.
...
O servidor de desenvolvimento sobe em `http://localhost:8080` (porta definida em `vite.config.ts`).
