-- Script idempotente para deixar um projeto Supabase existente compativel
-- com a aplicacao atual. Pode ser executado em um banco que ja tenha apenas
-- ir_parametros, ir_faixas e calculos, ou em um banco vazio.
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_type
    WHERE typnamespace = 'public'::regnamespace
      AND typname = 'tipo_declaracao_enum'
  ) THEN
...
CREATE TABLE IF NOT EXISTS public.ir_parametros (
  ano_calendario INT PRIMARY KEY,
  teto DECIMAL NOT NULL,
  inicio_correcao DATE NOT NULL
);

CREATE TABLE IF NOT EXISTS public.ir_faixas (
...
CREATE INDEX IF NOT EXISTS idx_ir_faixas_ano ON public.ir_faixas(ano_calendario);

CREATE TABLE IF NOT EXISTS public.calculos (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tipo_calculo public.tipo_calculo_enum NOT NULL,
  ano_calendario INT NOT NULL,
  numero_processo TEXT NOT NULL,
  nome_autor TEXT NOT NULL,
  tipo_declaracao public.tipo_declaracao_enum NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.indices_economicos (
  ...
  descricao TEXT,
  natureza TEXT NOT NULL CHECK (natureza IN ('CORRECAO', 'JUROS')),
  criado_em TIMESTAMPTZ DEFAULT now()
);

CREATE TABLE IF NOT EXISTS public.taxas_historicas (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  id_indice UUID NOT NULL REFERENCES public.indices_economicos(id),
  data_referencia DATE NOT NULL,
  valor_percentual NUMERIC(10, 6) NOT NULL,
  fator_multiplicador NUMERIC(16, 10) NOT NULL,
  fator_acumulado NUMERIC(20, 12) NOT NULL,
  criado_em TIMESTAMPTZ DEFAULT now(),
  UNIQUE (id_indice, data_referencia)
);

CREATE INDEX IF NOT EXISTS idx_taxas_indice_data
ON public.taxas_historicas (id_indice, data_referencia DESC);

CREATE TABLE IF NOT EXISTS public.templates_calculo (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  nome TEXT NOT NULL,
  descricao TEXT,
...
CREATE TABLE IF NOT EXISTS public.regras_subperiodo (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  id_template UUID NOT NULL REFERENCES public.templates_calculo(id) ON DELETE CASCADE,
